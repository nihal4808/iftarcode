...
async function createWebRtcTransport(router: types.Router) {
    return new Promise<types.WebRtcTransport>(async (resolve, reject) => {
        try {
...
io.on('connection', (socket: Socket) => {
    ...
    const emitToRoom = (event: string, data: any, excludePeerId?: string) => {
    ...
    socket.on('joinRoom', async ({ roomId, userName }: { roomId: string, userName: string }, callback) => {
